...
[![Tests](https://img.shields.io/badge/tests-206_passing-brightgreen)](#테스트)
...
Meta 광고 라이브러리(공개 투명성 페이지)에서 키워드나 페이지 이름으로 집행 중인
광고를 검색해 레퍼런스로 모아봅니다. 광고 문구, CTA, 크리에이티브 썸네일, 게재
시작일, 노출 플랫폼, 유사 소재 수가 카드로 표시되고, 카드를 누르면 광고
라이브러리 원본 페이지가 열립니다. 검색 결과에 등장한 광고주 페이지가 칩으로
집계되며, 칩을 누르면 해당 페이지의 광고만 모아볼 수 있습니다. 아래 **더 보기**
버튼으로 다음 페이지를 계속 이어 받아 수십~수백 건까지 누적할 수 있습니다.
국가(한국, 미국, 일본, 대만, 전체), 게재 상태(게재 중, 종료됨, 전체), 미디어
형식(영상, 이미지) 필터를 지원합니다.

- 데이터 소스: Meta Ad Library 공개 검색 페이지(첫 페이지, 임베디드 JSON) +
  `AdLibrarySearchPaginationQuery` GraphQL(더 보기), 무인증
...
- 제한: 비공식 경로라 메타 정책·doc_id 변경 시 영향 가능
...
현재 206개 테스트가 등록되어 있습니다. 외부 API 호출은 전부 mock 처리되어
오프라인에서도 실행됩니다.
